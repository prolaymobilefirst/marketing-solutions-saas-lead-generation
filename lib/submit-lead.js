...
/* Server-side proxy to the Make.com webhook. The browser never calls
   Make.com directly, and never receives the PDF — that's delivered solely
   by the automated email Make.com sends on a genuine webhook success. */
...
function validatePayload(body) {
  ...
  return {
    ...
    /* Always "Oui": forwardToMake only runs on a genuine lead submission,
       and Make.com only emails the PDF on that same successful call, so
       every row that reaches the sheet corresponds to a report having
       been sent. */
    rapport: 'Oui',
