...
**Results logic**: The results page shows 1, 3 or 5 recommended software cards, looked up directly from `content/affiliate-links.json`, which has three buckets: `autre` (Indy/Tiime/Abby/Shine/Qonto, 5 slots — simple/freelance invoicing tools), `crm` (Karlia/Axonaut/Qonto, 3 slots — CRM / "gestion commerciale tout-en-un" tools; Qonto is also in `autre` since its "compte pro + facturation" pitch fits both segments), and `compta` (Pennylane, 1 slot — automated accounting/declarations). **Priority drives routing before current software**: in `buildRecommendations()` (`js/webhook.js`), Step 2's priority answer (`quiz_volume`) is checked first — `gestion_crm` ("un outil tout-en-un pour ma gestion commerciale") always shows the `crm` bucket regardless of Step 3; `automatiser_compta` ("automatiser ma comptabilité et mes déclarations URSSAF/TVA") shows the `compta` bucket (Pennylane) *unless* Step 3's `quiz_connexion` answer is already `pennylane` — a visitor who already uses Pennylane shouldn't be shown an ad for it, so that specific combination falls through to the `crm` bucket instead. Otherwise the Step 3 `quiz_connexion` answer (current accounting software) picks the bucket via `CONNEXION_BUCKET`: `autre` maps to its own bucket; `pennylane`, `sage`, `cegid` and `ebp` all map to `crm` (Sage/Cegid/EBP as the closest analog to their existing accounting software; Pennylane as a complementary CRM upsell for visitors who didn't ask to automate their accounting specifically). 100% Particuliers (B2C) clients skip Steps 2-3 and see a dedicated out-of-scope banner instead of the recommendation grid. This mapping/lookup lives entirely in `js/webhook.js`'s `buildRecommendations()`/`buildCardHTML()` — no server-side logic. `.articles-grid` gets an `articles-grid--three` modifier class when 3 cards render (both `crm` and, if ever slotted to 3, `autre`), or `articles-grid--single` when exactly 1 renders (currently only the `compta` bucket) to center the lone card instead of leaving it flush left. One entry per bucket may be flagged `"topPick": true` in the JSON — it renders with a thicker amber border (`.article-card--top`) and a "★ Meilleur choix" corner ribbon (`.top-pick-ribbon`) to reduce choice paralysis on a multi-card grid; currently Indy (`autre`) and Karlia (`crm`) — `compta` has no topPick since a single-card bucket has no choice to disambiguate. The admin panel's "Liens affiliés" section (`admin/sections/affiliates.php`) edits this same file/bucket structure, including the top-pick checkbox — its `CONNEXION_BUCKETS` constant must stay in sync with `CONNEXION_BUCKET`/`COMPTA_PRIORITY_VALUE` in `js/webhook.js` (bucket keys and per-bucket slot counts).
