...
**Results logic**: The results page shows 2 or 3 recommended software cards, looked up directly from `content/affiliate-links.json`, which has exactly two buckets: `autre` (Indy/Tiime/Abby, 3 slots — simple/freelance invoicing tools) and `crm` (Karlia/Axonaut, 2 slots — CRM / "gestion commerciale tout-en-un" tools). **Priority overrides current software**: in `buildRecommendations()` (`js/webhook.js`), if Step 2's priority answer (`quiz_volume`) is `gestion_crm` ("un outil tout-en-un pour ma gestion commerciale"), the `crm` bucket is shown regardless of Step 3's answer — asking for a CRM and getting basic invoicing tools back is treated as a routing bug, not acceptable. Otherwise the Step 3 `quiz_connexion` answer (current accounting software) picks the bucket via `CONNEXION_BUCKET`: `autre` maps to its own bucket; `pennylane`, `sage`, `cegid` and `ebp` all map to `crm` (Pennylane already covers core accounting, so CRM tools are the complementary upsell rather than duplicating basic invoicing; Cegid was never given its own set, so it rides the closest analog). 100% Particuliers (B2C) clients skip Steps 2-3 and see a dedicated out-of-scope banner instead of the recommendation grid. This mapping/lookup lives entirely in `js/webhook.js`'s `buildRecommendations()`/`buildCardHTML()` — no server-side logic. `.articles-grid` gets an `articles-grid--three` modifier class when 3 cards render. One entry per bucket may be flagged `"topPick": true` in the JSON — it renders with a thicker amber border (`.article-card--top`) and a "★ Meilleur choix" corner ribbon (`.top-pick-ribbon`) to reduce choice paralysis on the 3-card grid; currently Indy (`autre`) and Karlia (`crm`). The admin panel's "Liens affiliés" section (`admin/sections/affiliates.php`) edits this same file/bucket structure, including the top-pick checkbox — its `CONNEXION_BUCKETS` constant must stay in sync with `CONNEXION_BUCKET` in `js/webhook.js` (bucket keys and per-bucket slot counts).
